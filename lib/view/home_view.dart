...
class HomeView extends StatelessWidget {
  ...
  Widget barChartWidget(HomeViewModel model, List<BarChartGroupData> barGroups,
      List<BarChartModel> data, BuildContext context) {
    return Column(
      children: [
        const SizedBox(height: Sizes.paddingAboveGraph),
        SizedBox(
          height: 250,
          child: Card(
            elevation: 0,
            ...
            child: Padding(
              padding: EdgeInsets.only(top: Sizes.smallPaddingWidget(context)),
              child: BarChart(
                BarChartData(
                  ...
                  barTouchData: BarTouchData(
                    ...
                    touchTooltipData: BarTouchTooltipData(
                      tooltipMargin: Sizes.paddingWidget(context) * 2,
                      getTooltipColor: (group) => Colors.white,
                      getTooltipItem: (group, groupIndex, rod, rodIndex) =>
                          BarTooltipItem(
                        rod.toY.toStringAsFixed(2).toString(),
                        TextStyle(
                          color: rod.color,
                        ),
                      ),
                  ...
                  titlesData: FlTitlesData(
                    ...
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                          showTitles: true,
                          reservedSize:
                              displayWidth(context) < 600 ? 28 : 28 * 1.5,
                          // changing bottom tile based on period selected for eg month,quarter and year
                          getTitlesWidget: (value, meta) =>
                              bottomTitles(value, meta, model, data, context)),
                    ),
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        reservedSize: Sizes.reservedSizeLeftTileWidget(context),
                        getTitlesWidget: (value, meta) =>
                  ...
                  groupsSpace: 4,
                  // showing data based on dropdown selected
                  // for eg amountwise and quantity wise
                  barGroups: barGroups,
                ),
        ...
        const SizedBox(height: Sizes.paddingBelowGraph),
      ],
    );
  }

  // bottom titles for quarterly graph will contain 4 values from jan to mar to oct to dec
  Widget bottomTitles(double value, TitleMeta meta, HomeViewModel model,
      List<BarChartModel> data, BuildContext context) {
    var style = TextStyle(
        color: Colors.grey[700],
        fontSize: Sizes.fontSizeGraphBottomTileWidget(context));
    String text;
    switch (value.toInt()) {
      case 0:
        text = data[value.toInt()].x;
        break;
      case 1:
        text = data[value.toInt()].x;
        break;
    ...
    return Sizes.leftTileSideTileWidget(meta.axisSide, text, style);
  }
  ...
  Widget leftTitles(double value, TitleMeta meta, BuildContext context) {
    ...
    var style = TextStyle(
      color: Colors.grey[700],
      fontSize: Sizes.fontSizeGraphLeftTileWidget(context),
    );
    return Sizes.leftTileSideTileWidget(
        meta.axisSide, meta.formattedValue, style);
  }
