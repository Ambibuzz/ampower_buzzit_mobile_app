...
class SalesInvoiceDetailView extends StatelessWidget {
  final String doctype;
  final String name;
  const SalesInvoiceDetailView(
      {Key? key, required this.doctype, required this.name})
      : super(key: key);
  static var globalKey =
      GlobalKey<FlutterMentionsState>(debugLabel: 'sales_invoice_detail_key');

  @override
  Widget build(BuildContext context) {
    return BaseView<SalesInvoiceDetailViewModel>(
      onModelReady: (model) async {
        var connectivityStatus =
            Provider.of<ConnectivityStatus>(context, listen: false);
        await model.getSalesInvoice(doctype, name, connectivityStatus);
        // await model.getTransitionState();
      },
      builder: (context, model, child) {
        return model.state == ViewState.busy
            ? WidgetsFactoryList.circularProgressIndicator()
            : salesInvoiceWidgetMobile(model, context);
      },
    );
  }

  Widget salesInvoiceWidgetMobile(
      SalesInvoiceDetailViewModel model, BuildContext context) {
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                Common.rowWidget('Customer', model.so.customer, context),
                Common.widgetSpacingVerticalSm(),
                Common.rowHtmlWidget(
                    'Customer Address', model.so.customerAddress, context),
                Common.widgetSpacingVerticalSm(),
                Common.rowWidget('Posting Date',
                    defaultDateFormat(model.so.postingDate!), context),
                Common.widgetSpacingVerticalSm(),
                Common.rowWidget(
                    'Due Date', defaultDateFormat(model.so.dueDate!), context),
                Common.widgetSpacingVerticalSm(),
                Common.rowWidget('Total Advance',
                    Others.formatter.format(model.so.totalAdvance), context),
                Common.widgetSpacingVerticalSm(),
                Common.rowWidget('Base Grand Total',
                    Others.formatter.format(model.so.baseGrandTotal), context),
                Common.widgetSpacingVerticalSm(),
                Common.rowWidget('Company Name', model.so.company, context),
                Common.widgetSpacingVerticalSm(),
                Common.rowHtmlWidget(
  ...
  Widget table(SalesInvoiceDetailViewModel model, BuildContext context) {
    // var columns = model.customerModel.salesTeam;
    var list = [];
    // model.so.items?.forEach((e) => list.add(e.toJson()));
    return JsonTable(
      list,
