...
class SalesOrderDetailViewModel extends BaseViewModel {
  ...
  Future getSalesOrder(String doctype, String name,
    ...
    try {
      if (connectivityStatus == ConnectivityStatus.cellular ||
        ...
        if (response != null) {
          ...
          so = SalesOrder.fromJson(response['docs'][0]);
        ...
        if (sodata != null) {
          ...
          if (solist.salesOrderList != null) {
            so = solist.salesOrderList!.firstWhere((e) => e.name == name);
          }
        }
      }
      notifyListeners();
    } catch (e) {
      exception(e, '', 'getSalesOrder');
    } finally {
